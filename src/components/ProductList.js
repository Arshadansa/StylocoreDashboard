...
import { v4 as uuidv4 } from "uuid"; // For unique image naming
...
const ProductList = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name.startsWith("dimensions.")) {
      const dimensionName = name.split(".")[1]; // Extract dimension name (length, width, height)
  ...
  const uploadColorImages = async (colorIndex) => {
    ...
    const updatedImages = [...color.images]; // Preserve existing images
    ...
    try {
      const updatedColors = await Promise.all(
        currentProduct.colors.map(async (color, colorIndex) => ({
          ...color,
          images: await uploadColorImages(colorIndex), // Merge existing images with new ones
        }))
      );
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="date"
                name="date_added"
                value={currentProduct.date_added?.substring(0, 10) || ""} // Extract date part from ISO string, default to empty string
                onChange={handleInputChange}
                className="w-full p-2 border rounded-lg"
              />
            </div>
